...
/**
 * Class which contains methods to manipulate the tricks of the game
 */
public final class PackedTrick {
    ...
    /**
     * Returns the next empty trick with the good trump
     ...
     * @return (boolean) : true if there are no cards during the trick and false otherwise
     */
    public static boolean isEmpty(int pkTrick) {
        return (card(pkTrick,0)==PackedCard.INVALID && card(pkTrick,1)==PackedCard.INVALID && card(pkTrick,2)==PackedCard.INVALID && card(pkTrick,3)==PackedCard.INVALID);
    }

    /**
     * Checks if the trick is full or not (number of cards) 
     * @param pkTrick (int) : the packed representation of the trick
     * @return (boolean) : true iff the trick is full, false otherwise
     */
    public static boolean isFull(int pkTrick) {
        return size(pkTrick)==MAX_CARD_PER_TRICK;
    }
    ...
    /**
     * Returns the index-th player that played  in the trick, i.e. if player 1 placed the second card of index 2 then 
    ...
    public static int withAddedCard(int pkTrick, int pkCard) {
        ...
        pkTrick = pkTrick & (~ Bits32.mask((BITS_FOR_CARD*trickSize), BITS_FOR_CARD));
     ...
     * @return (long): all the trump cards from the hand that are higher than the highest trump card in the trick
     */
